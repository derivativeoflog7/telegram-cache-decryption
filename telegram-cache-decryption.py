...
#On Linux, these should usually be in ~/.local/share/TelegramDesktop/tdata/
KEY_DATAS_PATH = "/home/davide/.local/share/TelegramDesktop/tdata/key_datas" # Path to key_datas file
USER_DATA_PATH = "/home/davide/.local/share/TelegramDesktop/tdata/user_data/" # path to user_data folder; if you have multiple accounts there's a folder for each account
...
def main():
    ...
    if KEY_DATAS_PATH is None or USER_DATA_PATH is None:
        print("You need to edit the .py file and declare where key_datas file and the user_data folder are")
        exit(1)
    LocalKey = read_key(KEY_DATAS_PATH)
    topdir = os.path.expanduser(USER_DATA_PATH)
